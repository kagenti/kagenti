...
SCRIPT_REL="./.github/scripts/local-setup/hypershift-full-test.sh"
if [ "$(git rev-parse --show-toplevel 2>/dev/null)" != "$REPO_ROOT" ]; then
    ...
    SCRIPT_REL="${WORKTREE_REL}/.github/scripts/local-setup/hypershift-full-test.sh"
fi
ENV_SOURCE="source .env.${MANAGED_BY_TAG}"
...
echo "  ${ENV_SOURCE} && ${SCRIPT_REL%/*}/show-services.sh ${CLUSTER_SUFFIX}"
echo ""
echo -e "${YELLOW}Access the hosted cluster:${NC}"
echo "  export KUBECONFIG=${HOSTED_KUBECONFIG_PATH}"
echo "  oc get nodes"
echo ""
echo -e "${YELLOW}Re-run tests only:${NC}"
echo "  ${ENV_SOURCE} && ${SCRIPT_REL} ${CLUSTER_SUFFIX} --skip-cluster-create --skip-cluster-destroy --include-test"
echo ""
echo -e "${YELLOW}Reinstall Kagenti (keep cluster):${NC}"
echo "  ${ENV_SOURCE} && ${SCRIPT_REL} ${CLUSTER_SUFFIX} --skip-cluster-create --clean-kagenti --skip-cluster-destroy"
echo ""
echo -e "${YELLOW}Destroy cluster:${NC}"
echo "  ${ENV_SOURCE} && ${SCRIPT_REL} ${CLUSTER_SUFFIX} --include-cluster-destroy"
echo ""
echo -e "${YELLOW}Full cycle (create → deploy → test → destroy):${NC}"
echo "  ${ENV_SOURCE} && ${SCRIPT_REL} ${CLUSTER_SUFFIX}"
echo ""
