...
def _get_auth_headers() -> dict:
    ...
    try:
        ...
        # Use kagenti-test-user (kagenti realm) — NOT keycloak-initial-admin (master realm).
        # The backend validates tokens against the kagenti realm, so the token must
        # be issued by that realm's IdP.
        for secret_name, ns in [
            ("kagenti-test-user", "keycloak"),
            ("keycloak-initial-admin", "keycloak"),
        ]:
            try:
                secret = api.read_namespaced_secret(name=secret_name, namespace=ns)
                if secret.data:
                    break
            except Exception:
                continue
        else:
            logger.info("No Keycloak credentials found — auth likely disabled")
            _cached_auth_headers = {}
            return _cached_auth_headers

        username = base64.b64decode(secret.data["username"]).decode("utf-8")
        password = base64.b64decode(secret.data["password"]).decode("utf-8")
        realm = base64.b64decode(secret.data.get("realm", b"a2FnZW50aQ==")).decode(
            "utf-8"
        )
    ...
    # Acquire token from the correct Keycloak realm
    ...
    client_id = (
        ...
        if "client_id" in (secret.data or {})
        else ""
    )
    client_secret = (
        base64.b64decode(secret.data["client_secret"]).decode("utf-8")
        if "client_secret" in (secret.data or {})
        else ""
    )
...
def _check_sandbox_api_available() -> bool:
    ...
    Uses VERIFY_SSL (default False) for SSL verification because this runs
    at module-import time before _get_ssl_context() helpers are available,
    and self-signed certs on HyperShift/OCP clusters would otherwise cause
    SSL errors that mask the real availability check.
    """
...
# Skip entire module if sandbox feature flag is not enabled (legitimate skip).
# But if sandbox IS enabled and the API is unreachable, fail — don't hide it.
pytestmark = [
    pytest.mark.requires_features(["sandbox"]),
]
