...
@pytest.fixture(scope="module")
def virtual_client(_ssl_verify):
...
class TestLiteLLMHealth:
    ...
    def test_readiness(self, _ssl_verify):
        resp = httpx.get(
            f"{LITELLM_PROXY_URL}/health/readiness", timeout=10, verify=_ssl_verify
        )
        assert resp.status_code == 200, f"Readiness check failed: {resp.text}"

    def test_liveliness(self, _ssl_verify):
        resp = httpx.get(
            f"{LITELLM_PROXY_URL}/health/liveliness", timeout=10, verify=_ssl_verify
        )
...
class TestLiteLLMModels:
    ...
    def test_maas_models_present(self, master_client):
        """At least one MaaS model is registered."""
        resp = master_client.get("/v1/models")
        model_ids = [m["id"] for m in resp.json()["data"]]
        # Models are auto-discovered from MaaS endpoint — check at least one exists
        maas_models = [
            m for m in model_ids if "llama" in m or "qwen" in m or "deepseek" in m
        ]
        assert len(maas_models) > 0, f"No MaaS models found in {model_ids}"
    ...
    def test_model_info(self, master_client):
        ...
        data = resp.json()["data"]
        assert len(data) >= 1, f"Expected >= 1 models, got {len(data)}"


class TestLiteLLMChatCompletions:
    """Chat completion through the proxy."""

    def test_chat_completion_llama(self, master_client):
        """Test chat completion with Llama (primary MaaS model)."""
    ...
    def test_chat_deepseek_qwen(self, master_client):
        """Test chat completion with DeepSeek R1 Distill Qwen.

        This is a reasoning model that may return content in the
...
class TestLiteLLMVirtualKeys:
    ...
    def test_invalid_key_rejected(self, _ssl_verify):
