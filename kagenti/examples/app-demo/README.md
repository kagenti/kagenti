...
A standalone demo application that shows how a third-party app integrates with the Kagenti platform: authenticate via Keycloak, discover deployed agents, and send tasks.

> **📖 For architecture details and development guidance**, see the [Developing a Kagenti Application Guide](../../../docs/developing-kagenti-app.md).

## Quick Start

**Prerequisites:**
- A running Kagenti Kind cluster with UI enabled
- At least one agent deployed in any namespace (e.g., follow the [Weather Agent Demo](https://github.com/kagenti/kagenti-extensions/blob/main/authbridge/demos/weather-agent/demo-ui.md))
- Users created in Keycloak (default users: `alice`, `bob`, `charlie`)

> **💡 Note:** The app-demo works with the `kagenti` realm in Keycloak and can discover agents in any namespace. You don't need a specific namespace like "team1" - the app dynamically lists all available namespaces with agents.
...
- Creates the `kagenti-operator` realm role
- Deploys the application to the `kagenti-system` namespace

> **⚠️ After deployment**, you must grant user access to agents before they appear in the app. See step 3 below.

### 2. Configure User Permissions

Assign the `kagenti-operator` role to users who should be able to send tasks to agents.

**Assign kagenti-operator role to users in Keycloak:**

1. Open `http://keycloak.localtest.me:8080/admin` and sign in
   - Get credentials: `.github/scripts/local-setup/show-services.sh`
2. Select the **kagenti** realm (dropdown in the top-left)
3. Go to **Users** → select a user (e.g., `alice`)
4. **Role mapping** tab → **Assign role**
5. Filter by realm roles → select **kagenti-operator** → **Assign**

> **💡 Tip:** Any authenticated user can browse agents. Only users with `kagenti-operator` can send tasks.
...
3. Log in with a user that has the `kagenti-operator` role (e.g., `alice`)
4. **Browse agents:**
...
**Troubleshooting:**
- If you see "Forbidden" errors, ensure the user has the `kagenti-operator` role and has logged out/in after role assignment
...
For automation or scripting, you can assign the `kagenti-operator` role via kubectl:
...
# Assign kagenti-operator to a user (replace alice with the target username)
kubectl exec -n keycloak keycloak-0 -- \
  /opt/keycloak/bin/kcadm.sh add-roles -r kagenti \
  --uname alice --rolename kagenti-operator \
...
| `kagenti-operator` | Send tasks to agents |
...
- Node.js 18+ with `npm`
- Access to a running Kagenti cluster (Kind or remote)
...
export KAGENTI_API_URL=http://kagenti-backend.localtest.me:8080  # Point to Kagenti Backend
...
3. Test the full flow: Frontend → Backend → Kagenti Backend
...
**Backend issues:**
- Check logs: `kubectl logs -n kagenti-system deployment/app-demo-backend -f`
- Verify Kagenti Backend is accessible: `curl http://kagenti-backend.kagenti-system.svc.cluster.local:8000/health`
...
- Verify user has `kagenti-operator` role in Keycloak
...
|----------|-------------|---------|
| `KAGENTI_API_URL` | Kagenti backend URL | `http://kagenti-backend.kagenti-system.svc.cluster.local:8000` |
| `KEYCLOAK_PUBLIC_URL` | Keycloak URL (browser) | `http://keycloak.localtest.me:8080` |
| `KEYCLOAK_REALM` | Keycloak realm | `kagenti` |
...
TOKEN_BROKER_URL=http://token-broker.kagenti-system.svc.cluster.local:8190
```
...
- **[Developing a Kagenti Application Guide](../../../docs/developing-kagenti-app.md)** - Architecture, design patterns, and best practices
- **[Identity Guide](../../../docs/identity-guide.md)** - Authentication and authorization details
- **[Kagenti Documentation](../../../README.md)** - Full platform documentation
