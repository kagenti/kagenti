...
Today Kagenti can sync skills from an **external** skillberry-store instance: the
...
implementation should follow existing Kagenti component patterns and avoid
...
- A background loop in the backend (`run_skill_autosync_loop` →
  `sync_skills_once`) reads the `kagenti-skill-autosync-config` ConfigMap in
  `kagenti-system`, and if `enabled: "true"` with a `registry-url`, polls
...
- **Crucially, the loop trusts the `registry-url` from the ConfigMap with no SSRF
  ...
  (`kagenti/backend/app/routers/skills.py:155`, called at lines 189/220).
  `_fetch_registry_skills` (`kagenti/backend/app/services/skill_autosync.py:82`)
...
2. Seed the `kagenti-skill-autosync-config` ConfigMap pointing at the in-cluster Service.
...
|----------|--------|-----------|
| Chart placement | `charts/kagenti` (not `kagenti-deps`) | Autosync bootstrap ConfigMap must live in the same chart as the consuming backend; skillberry-store is skills-feature-specific, not generic infra. Single chart, single flag path. |
| Auto-enable mechanism | Helm-rendered `kagenti-skill-autosync-config` ConfigMap | Reuses the existing autosync-via-ConfigMap mechanism; no backend change. |
...
*Alternative considered:* place the Deployment in `charts/kagenti-deps` to match the
mlflow/keycloak backing-service precedent. Rejected: forces cross-chart URL
coordination (bootstrap ConfigMap in `kagenti`, Deployment in `kagenti-deps`) for
no real benefit.

## Changes

### 1. `charts/kagenti/values.yaml`
...
### 2. `charts/kagenti/templates/skillberry-store.yaml`
...
### 3. `charts/kagenti/templates/skillberry-autosync-configmap.yaml`
...
kind: ConfigMap
metadata:
  name: kagenti-skill-autosync-config   # name the backend loop already reads
...
### 4. `scripts/kind/setup-kagenti.sh`

`--with-skills` already enables `featureFlags.skills` + `featureFlags.externalSkills`
and auto-enables UI+backend. Add to `KAGENTI_FLAGS` when `WITH_SKILLS`:

```bash
KAGENTI_FLAGS+=(--set components.skillberryStore.enabled=true)
...
- **Kind installer E2E:** extend the flag-detection map in
  `kagenti/tests/e2e/common/test_kind_installer.py` so `--with-skills` is verified
  to produce a `skillberry-store` Deployment in `kagenti-system`.

## Known limitations

- **Pre-existing ConfigMap adoption:** on an existing cluster where a user already
  created `kagenti-skill-autosync-config` by hand (via UI/API), a fresh Helm install
...
- A new dedicated `kagenti_feature_flag_*` (the existing `externalSkills` flag gates behavior).
