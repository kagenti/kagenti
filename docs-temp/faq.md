...
No. Rosso is framework-neutral — LangGraph, CrewAI, AG2, AutoGen, or a custom loop all work, as long as the agent speaks [A2A](concepts/agents.md). See [Bring your own agent](guides/bring-your-own-agent.md).

### How is this different from an agent framework?

Frameworks help you *build* an agent. Rosso is the platform that *runs* it — identity, governance, isolation, and observability on Kubernetes. It sits beneath your agent, not in place of it.

### Do agents need long-lived API keys?

No. Rosso issues cryptographic workload identity and short-lived, scoped tokens at runtime, so agents rarely need standing secrets. See [Security & Identity](security/overview.md).
...
The platform is open source. Your real cost is infrastructure and model tokens — and Rosso attributes [token cost](observability/metrics-and-cost.md) per agent and team so you can see and control it.

### Why "Rosso" — I've seen "Kagenti"?

The project is being renamed from Kagenti to Rosso. You may see both names during the transition; they refer to the same platform.
